...
// Copyright (c) 2009-2017 The Aiccoin Core developers
...
//开发区块浏览器关键是能从钱包获取数据
//数据可以通过调用钱包注册好的RPC命令获得
//数据获取到了后，有两种处理方式，将数据送到web服务器上
//1、直接将本程序代码做成组件，使之成为web服务器的一部分（建议用这种方式，原因是避免了再次通信的情况）
//2、将获取到的数据，通过网络传送到web服务器上

std::string cmd="getblockchaininfo";// RPC 命令

static const char DEFAULT_RPCCONNECT[] = "127.0.0.1";// 钱包所在主机的ip地址
...
static void SetupCliArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", AICCOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to aiccoind)", false, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\nUsage:\n"
                  "  aiccoin-cli [options] <command> [params]  " + strprintf("Send command to %s", PACKAGE_NAME) + "\n" +
                  "  aiccoin-cli [options] -named <command> [name=value] ... " + strprintf("Send command to %s (with named arguments)", PACKAGE_NAME) + "\n" +
                  "  aiccoin-cli [options] help                List commands\n" +
                  "  aiccoin-cli [options] help <command>      Get help for a command\n";
...
public:
    ...
    UniValue PrepareRequest(const std::string& method, const std::vector<std::string>& args) override
    {
        ...
        //result.push_back(JSONRPCRequestObj("generate 20", NullUniValue, ID_HASHINFO));
        return result;
    }
...
static UniValue CallRPC(BaseRequestHandler *rh, const std::string& strMethod, const std::vector<std::string>& args)
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\nMake sure the aiccoind server is running and that you are connecting to the correct RPC port.", host, port, responseErrorMessage));
    } else if (response.status == HTTP_UNAUTHORIZED) {
        if (failedToGetAuthCookie) {
            throw std::runtime_error(strprintf(
                "Could not locate RPC credentials. No authentication cookie could be found, and RPC password is not set.  See -rpcpassword and -stdinrpcpass.  Configuration file: (%s)",
                GetConfigFile(gArgs.GetArg("-conf", AICCOIN_CONF_FILENAME)).string().c_str()));
        } else {
...
static int CommandLineRPC(int argc, char *argv[], UniValue& result)
{
    std::string strPrint;
    int nRet = 0;
    try {
                ...
                if (!error.isNull()) {
                    ...
                    if (error.isObject())
                    {
                        ...
                        if (errCode.isNum() && errCode.get_int() == RPC_WALLET_NOT_SPECIFIED) {
                            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to aiccoin-cli command line.";
                        }
                    }
                } else {
                    // Result
                    if (result.isNull())
                        ...
                        strPrint = result.get_str();//此处为正常情况下RPC响应获取到的数据，若要将数据发送出去，可从此处获取数据
                    else
    ...
    //print the messages obtained by RPC
    if (strPrint != "") {
...
int aiccoin_cli(int argc,char* argv[],UniValue& result)
{
    SetupEnvironment();
    if (!SetupNetworking()) {
    ...
    try {
        ret = CommandLineRPC(argc, argv, result);
        gArgs.ClearArgs();
    }
    catch (const std::exception& e) {
